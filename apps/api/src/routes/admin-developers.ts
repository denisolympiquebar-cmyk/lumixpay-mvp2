...
const router = Router();
const guard  = [authenticate, requireRole("admin")] as const;
...
router.get("/:user_id", ...guard, async (req, res) => {
  ...
  try {
    const [userRes, keysRes, webhooksRes, usageRes] = await Promise.all([
      pool.query(
        "SELECT id, email, full_name, username, role, created_at FROM users WHERE id = $1",
        [user_id]
      ),
      pool.query(
      ...
      pool.query(
        `SELECT route, method, COUNT(*) AS requests,
...
router.post("/webhooks/:id/disable", ...guard, async (req, res) => {
  try {
    const { rowCount } = await pool.query(
      "UPDATE webhooks SET status = 'disabled' WHERE id = $1 AND status = 'active'",
      [req.params["id"]]
    );
    if (!rowCount) return res.status(404).json({ error: "Webhook not found or already disabled" });
    return res.json({ ok: true });
  } catch (err) {
