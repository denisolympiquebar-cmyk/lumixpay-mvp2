...
router.post("/", authenticate, async (req, res) => {
  ...
  try {
    ...
    const { rows: userRows } = await pool.query<{ id: string; email: string; full_name: string; username: string | null }>(
      "SELECT id, email, full_name, username FROM users WHERE (email = $1 OR username = $1) AND role != 'system'",
      [identifier]
    );
    if (!userRows[0]) {
    ...
    const { rows } = await pool.query(
      `INSERT INTO contacts (owner_user_id, contact_user_id, nickname)
       ...
       RETURNING *`,
      [req.user!.sub, contactUser.id, nickname ?? null]
    );
