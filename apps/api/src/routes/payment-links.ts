...
import { Account, PaymentLink } from "../db/types";
...
const ClaimSchema = z.object({
  amount: z.number().positive().optional(), // required if link has no fixed amount
});
...
router.post("/", authenticate, idempotent, async (req, res) => {
  ...
  try {
    const { rows } = await pool.query<PaymentLink>(
      `INSERT INTO payment_links (creator_user_id, asset_id, amount, description, max_uses, expires_at)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
      [req.user!.sub, asset_id, amount ?? null, description ?? null, max_uses ?? null, expires_at ?? null]
    );
...
router.get("/", authenticate, async (req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT pl.*, a.currency_code, a.display_symbol
...
router.patch("/:id/disable", authenticate, async (req, res) => {
  try {
    const { rowCount } = await pool.query(
      "UPDATE payment_links SET status = 'disabled' WHERE id = $1 AND creator_user_id = $2",
      [req.params["id"], req.user!.sub]
    );
    if (!rowCount) return res.status(404).json({ error: "Payment link not found" });
...
router.get("/pay/:id", async (req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT pl.*, a.currency_code, a.display_symbol, a.display_name,
    ...
    if (!rows[0]) return res.status(404).json({ error: "Payment link not found" });
    const link = rows[0];
    if (link.status !== "active") return res.status(410).json({ error: "Payment link is no longer active" });
...
router.post("/pay/:id/claim", authenticate, idempotent, async (req, res) => {
  ...
  try {
    const { rows: linkRows } = await pool.query<PaymentLink>(
    ...
    const { rows: payerRows } = await pool.query<Account>(
      "SELECT * FROM accounts WHERE user_id = $1 AND asset_id = $2 AND label = 'main'",
      [req.user!.sub, link.asset_id]
    );
    const { rows: creatorRows } = await pool.query<Account>(
      "SELECT * FROM accounts WHERE user_id = $1 AND asset_id = $2 AND label = 'main'",
      [link.creator_user_id, link.asset_id]
    );
    if (!payerRows[0]) return res.status(404).json({ error: "Payer account not found for this asset" });
    if (!creatorRows[0]) return res.status(404).json({ error: "Creator account not found for this asset" });
