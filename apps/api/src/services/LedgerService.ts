...
export class LedgerService {
  ...
  async cancelApprovedWithdrawal(params: {
    ...
    const result = await withTransaction(async (client) => {
      ...
      if (!wr) {
        throw Object.assign(
          new Error(`Withdrawal request ${withdrawalId} not found`),
          { status: 404 }
        );
      }
      ...
      if (wr.status !== "approved") {
        throw Object.assign(
          new Error(
            `Cannot cancel withdrawal in status '${wr.status}'. ` +
            `Only 'approved' withdrawals without an on-chain confirmation can be cancelled.`
          ),
          { status: 409, code: "INVALID_STATUS_FOR_CANCEL" }
        );
      }
      ...
      if (wr.xrpl_confirmed_at !== null || wr.xrpl_tx_hash !== null) {
        throw Object.assign(
          new Error(
            "Cannot cancel an already-confirmed withdrawal " +
            "(xrpl_confirmed_at or xrpl_tx_hash is set). " +
            "Contact the XRPL issuer to reverse the on-chain payment if needed."
          ),
          { status: 409, code: "WITHDRAWAL_ALREADY_CONFIRMED" }
        );
      }
      ...
      if (wr.xrpl_submitted_at !== null) {
        throw Object.assign(
          new Error(
            "Settlement is already in progress for this withdrawal. " +
      ...
      const { rows: updated } = await client.query<WithdrawalRequest>(
        `UPDATE withdrawal_requests
          ...
          WHERE id = $3
          RETURNING *`,
        [reason ?? "Cancelled by admin", adminId, withdrawalId]
      );
    ...
    void (async () => {
      try {
        ...
        await notificationService.create({
          ...
          body:
            `Your withdrawal of ${result.net_amount} ${currency}_TEST was cancelled by an admin. ` +
  ...
  async settleWithdrawal(params: SettleWithdrawalParams): Promise<WithdrawalRequest> {
    ...
    const settled = await withTransaction(async (client) => {
      ...
      if (wr.status !== "approved") {
        throw Object.assign(
          new Error(
            `Cannot complete settlement: withdrawal is in status '${wr.status}'. ` +
            `It may have been cancelled during on-chain processing.`
          ),
          { status: 409, code: "WITHDRAWAL_NO_LONGER_APPROVED" }
        );
      }
    ...
    void notificationService
      .create({
        ...
        body:
          `Your withdrawal of ${pre.net_amount} ${currency}_TEST has been sent to ` +
    ...
    void (async () => {
      try {
        const { rows: walletRows } = await pool.query<{ user_id: string }>(
          `SELECT user_id
             FROM user_wallets
