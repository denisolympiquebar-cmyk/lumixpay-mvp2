...
export type TxHistoryResult =
  | { status: "ok";                       walletAddress: string; transactions: WalletTransaction[] }
...
function rippleEpochToIso(xrplDate: number): string {
...
function mapCurrencyCode(
  ...
  if (upper === rlusdHex) return rlusdLabel;
  if (upper === eurqHex)  return eurqLabel;
  if (code.length === 3)  return code;   // 3-char ISO code (e.g. "USD", "XRP") — pass through
  return null;                            // unknown hex currency — caller decides to skip
}

// ── Raw XRPL account_tx response types ───────────────────────────────────────
...
export class XrplTransactionHistoryService {
  /**
  ...
  async getTransactionHistory(
    ...
    const rlusdHex   = currencyToHex(rlusdCode).toUpperCase();
    const eurqHex    = currencyToHex(eurqCode).toUpperCase();
    const rlusdLabel = `${rlusdCode}_TEST`;
    const eurqLabel  = `${eurqCode}_TEST`;

    // ── Fetch raw transactions from XRPL Testnet ──────────────────────────
    let rawEntries: XrplTxEntry[];
    try {
      rawEntries = await withRetry(
        () => this._fetchAccountTx(address, rpcUrl, limit),
        isXrplTransientError,
        5,
        2_000,
        (attempt, err) =>
          console.warn(
            `[TxHistory] account_tx retry (${attempt}/5) for ${address}: ` +
    ...
    for (const entry of rawEntries) {
      ...
      if (txType === "Payment") {
        ...
        if (!txAccount || !txDestination) continue;
        ...
        if (typeof txAmount === "string") {
          ...
          if (amtIssuer !== issuerAddress) continue;

          const label = mapCurrencyCode(amtCurrency, rlusdHex, eurqHex, rlusdLabel, eurqLabel);
          if (!label) continue; // unknown currency — skip to avoid confusion
      ...
      // ── TrustSet ───────────────────────────────────────────────────────
        ...
        if (!txAccount || txAccount !== address) continue;
        if (!limitAmount || typeof limitAmount !== "object") continue;

        const limitIssuer   = limitAmount.issuer   ?? "";
        const limitCurrency = limitAmount.currency ?? "";

        if (limitIssuer !== issuerAddress) continue;

        const trustLineCurrency =
          mapCurrencyCode(limitCurrency, rlusdHex, eurqHex, rlusdLabel, eurqLabel) ?? undefined;
  ...
  private async _fetchAccountTx(
    ...
    if (status !== 200) throw new Error(`account_tx HTTP ${status}`);
...
export const xrplTransactionHistoryService = new XrplTransactionHistoryService();
