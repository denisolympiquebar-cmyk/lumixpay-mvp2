...
export class WalletEncryptionService {
  ...
  constructor(masterKeyBase64url: string, keyId: string) {
    if (!masterKeyBase64url) {
      throw new Error(
        "WalletEncryptionService: WALLET_MASTER_KEY is not set. " +
    ...
    if (decoded.length !== 32) {
      throw new Error(
        `WalletEncryptionService: WALLET_MASTER_KEY must decode to exactly 32 bytes ` +
  ...
  decrypt(encoded: string): string {
    ...
    if (parts.length !== 4) {
      throw new Error(
        `WalletEncryptionService: malformed envelope — expected 4 colon-separated parts, ` +
        `got ${parts.length}.`
      );
    }
    ...
    const [_keyId, ivB64, tagB64, ctB64] = parts as [string, string, string, string];
    ...
    try {
      ...
      throw new Error(
        "WalletEncryptionService: decryption failed — invalid key, wrong key version, or tampered data."
      );
    }
  }
}
