...
export class XrplSettlementQueueService {
  ...
  private async _processOne(withdrawalId: string): Promise<void> {
    ...
    try {
      ...
      console.log(
        `[SettlementQueue] ✓ Settled ${withdrawalId} ` +
      ...
      console.error(
        `[SettlementQueue] ✗ Failed to settle ${withdrawalId}: [${code}] ${message}`
      );
   ...
   */
  private _getLedger(): LedgerService {
    if (!this._ledger) {
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      this._ledger = require("./LedgerService").ledgerService as LedgerService;
    }
    return this._ledger;
  }
}

export const xrplSettlementQueueService = new XrplSettlementQueueService();
