...
const FeeService_1 = require("./FeeService");
...
class LedgerService {
    ...
    payments;
    settlement;
    constructor(fees = FeeService_1.feeService, payments = MockPaymentProvider_1.mockPaymentProvider, 
    ...
    // ────────────────────────────────────────────────────────────────────────
    settlement = xrpl_1.settlementProvider) {
        this.fees = fees;
     ...
     */
    async postEntry(client, params) {
        const { idempotencyKey, debitAccountId, creditAccountId, assetId, amount, entryType, referenceId, referenceType, metadata, debitField = "available", creditField = "available", } = params;
       ...
       RETURNING *`, [
        ...
        const debitRes = await client.query(`UPDATE balances
          SET ${debitField} = ${debitField} - $1, updated_at = NOW()
        WHERE account_id = $2`, [amount.toFixed(6), debitAccountId]);
        ...
        const creditRes = await client.query(`UPDATE balances
          SET ${creditField} = ${creditField} + $1, updated_at = NOW()
        WHERE account_id = $2`, [amount.toFixed(6), creditAccountId]);
    ...
    async assertSufficientAvailable(client, accountId, required) {
        const { rows } = await client.query("SELECT available FROM balances WHERE account_id = $1 FOR UPDATE", [accountId]);
        if (!rows[0])
            throw new Error(`Balance row not found for account ${accountId}`);
        const available = new decimal_js_1.default(rows[0].available);
        if (available.lt(required)) {
            throw new Error(`Insufficient balance. Available: ${available.toFixed(6)}, Required: ${required.toFixed(6)}`);
        }
    }
    // ── Private: system account lookup ─────────────────────────────────────────
    systemAccounts(assetId) {
    ...
    async topUp(params) {
        ...
        const result = await (0, pool_1.withTransaction)(async (client) => {
            ...
            await TreasuryService_1.treasuryService.ensureCanIssue(client, assetId, gross);
            // Insert topup record (pending)
            const { rows: topupRows } = await client.query(`INSERT INTO topup_transactions
         ...
         RETURNING *`, [
            ...
            await TreasuryService_1.treasuryService.consume(client, assetId, gross);
            // Mark topup completed
            const { rows: completedRows } = await client.query(`UPDATE topup_transactions
    ...
    async transfer(params) {
        ...
        const gross = new decimal_js_1.default(params.grossAmount);
        const { fee, net } = this.fees.compute(gross);
        const sys = this.systemAccounts(assetId);
        ...
        const result = await (0, pool_1.withTransaction)(async (client) => {
            ...
            const { rows: transferRows } = await client.query(`INSERT INTO transfers
        ...
        void (async () => {
            try {
                ...
                const [senderRes, recipRes] = await Promise.all([
                    pool_1.pool.query("SELECT user_id FROM accounts WHERE id = $1", [fromAccountId]),
                    pool_1.pool.query("SELECT user_id FROM accounts WHERE id = $1", [toAccountId]),
                ]);
                ...
                const idsToFetch = [senderUserId, recipientUserId].filter((id) => !!id);
                const usernameMap = new Map();
                if (idsToFetch.length > 0) {
                    const { rows: userRows } = await pool_1.pool.query("SELECT id, username FROM users WHERE id = ANY($1::uuid[])", [idsToFetch]);
                    for (const u of userRows)
                        usernameMap.set(u.id, u.username);
                }
                const formatIdentity = (userId) => {
    ...
    async requestWithdrawal(params) {
        const { userId, accountId, assetId, xrplDestinationAddress, xrplDestinationTag } = params;
        const gross = new decimal_js_1.default(params.grossAmount);
        const { fee, net } = this.fees.compute(gross);
        const sys = this.systemAccounts(assetId);
        ...
        const result = await (0, pool_1.withTransaction)(async (client) => {
            ...
            const { rows: wRows } = await client.query(`INSERT INTO withdrawal_requests
           (id, user_id, account_id, asset_id, gross_amount, fee_amount, net_amount,
         ...
         RETURNING *`, [
            ...
            await client.query(`UPDATE balances SET locked = locked + $1, updated_at = NOW() WHERE account_id = $2`, [net.toFixed(6), accountId]);
            return wRows[0];
        });
        void NotificationService_1.notificationService
            .create({
        ...
        void StreamService_1.streamService.publishAdmin("admin.withdrawals.updated", { userId });
        return result;
    }
    ...
    async reviewWithdrawal(params) {
        ...
        const result = await (0, pool_1.withTransaction)(async (client) => {
            ...
            SET status     = $1,
                ...
                updated_at  = NOW()
          WHERE id = $4
          RETURNING *`, [decision === "approve" ? "approved" : "rejected", adminId, note ?? null, withdrawalId]);
            if (decision === "reject") {
    ...
    async getAccountsWithBalances(userId) {
       ...
       WHERE a.user_id = $1
         AND a.label = 'main'
       ORDER BY ast.currency_code`, [userId]);
        return rows;
    }
    // ── Public: getLedgerHistory ─────────────────────────────────────────────────
    async getLedgerHistory(accountId, limit = 50, offset = 0) {
        const { rows } = await pool_1.pool.query(`SELECT * FROM ledger_entries
    ...
    async settleWithdrawal(params) {
        ...
        const { rows: preRows } = await pool_1.pool.query("SELECT * FROM withdrawal_requests WHERE id = $1", [withdrawalId]);
        const pre = preRows[0];
        if (!pre) {
        ...
        if (pre.status !== "approved") {
            throw Object.assign(new Error(`Cannot settle withdrawal in status '${pre.status}'. ` +
                `Withdrawal must be in 'approved' status.`), { status: 409, code: "INVALID_STATUS_FOR_SETTLEMENT" });
        }
        ...
        if (pre.xrpl_submitted_at !== null && pre.xrpl_confirmed_at === null) {
            throw Object.assign(new Error("Settlement already submitted and awaiting provider confirmation. " +
                "Do not retry until the current attempt has resolved."), { status: 409, code: "SETTLEMENT_IN_FLIGHT" });
        }
        ...
        await pool_1.pool.query(`UPDATE withdrawal_requests
          SET settlement_provider = $1,
              ...
              updated_at          = NOW()
        WHERE id = $2 AND status = 'approved' AND xrpl_submitted_at IS NULL`, [providerName, withdrawalId]);
        // ── 3. Fetch issuer address for the asset (needed by XRPL provider) ───────
        const { rows: assetRows } = await pool_1.pool.query("SELECT issuer_address FROM assets WHERE id = $1", [pre.asset_id]);
        const issuerAddress = assetRows[0]?.issuer_address ?? "";
        const net = new decimal_js_1.default(pre.net_amount);
        ...
        const settled = await (0, pool_1.withTransaction)(async (client) => {
          ...
          WHERE account_id = $2`, [net.toFixed(6), wr.account_id]);
            ...
            const { rows: settledRows } = await client.query(`UPDATE withdrawal_requests
            SET status               = 'settled',
                xrpl_tx_hash         = $1,
                xrpl_confirmed_at    = $2,
     ...
     */
    async publishBalancesAndActivity(userId, accountId, entry) {
        try {
            const { rows } = await pool_1.pool.query(`SELECT a.id AS account_id, a.label, b.available, b.locked, ast.currency_code, ast.display_symbol
           FROM accounts a
           JOIN balances b  ON b.account_id = a.id
           JOIN assets ast  ON ast.id        = a.asset_id
          WHERE a.user_id = $1 AND a.label = 'main'`, [userId]);
            StreamService_1.streamService.publish(userId, "balances.updated", { accounts: rows });
            if (entry) {
                StreamService_1.streamService.publish(userId, "activity.new", {
                    entry: {
                        id: entry.id,
