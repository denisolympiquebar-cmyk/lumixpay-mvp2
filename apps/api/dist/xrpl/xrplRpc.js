...
exports.sleep = sleep;
exports.isXrplTransientError = isXrplTransientError;
exports.withRetry = withRetry;
exports.assertTestnetRpc = assertTestnetRpc;
...
function postJson(url, body, timeoutMs = 15_000) {
    return new Promise((resolve, reject) => {
        ...
        req.setTimeout(timeoutMs, () => req.destroy(new Error(`postJson timed out after ${timeoutMs}ms: ${url}`)));
        req.on("error", reject);
        req.write(bodyBuf);
        req.end();
    });
 ...
 */
function isXrplTransientError(err) {
    if (!(err instanceof Error))
        return false;
    const msg = err.message;
    return (msg.includes("noNetwork") ||
 ...
 *   );
 */
async function withRetry(fn, shouldRetry, maxAttempts, delayMs, onRetry) {
    let lastErr;
