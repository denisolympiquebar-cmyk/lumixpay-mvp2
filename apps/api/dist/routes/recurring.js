...
const CreatePlanSchema = zod_1.z.object({
    ...
    interval: zod_1.z.enum(["weekly", "monthly"]),
    day_of_week: zod_1.z.number().int().min(0).max(6).optional(),
    day_of_month: zod_1.z.number().int().min(1).max(28).optional(),
    description: zod_1.z.string().max(255).optional(),
});
const SubscribeSchema = zod_1.z.object({
    start_at: zod_1.z.string().datetime().optional(), // default = now
});
...
router.post("/plans", auth_1.authenticate, async (req, res) => {
    ...
    try {
        const { rows } = await pool_1.pool.query(`INSERT INTO recurring_plans (creator_user_id, asset_id, amount, interval, day_of_week, day_of_month, description)
       VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`, [req.user.sub, asset_id, amount, interval, day_of_week ?? null, day_of_month ?? null, description ?? null]);
...
router.get("/plans", auth_1.authenticate, async (req, res) => {
    try {
        const { rows } = await pool_1.pool.query(`SELECT rp.*, a.currency_code, a.display_symbol,
        ...
        ORDER BY rp.created_at DESC`, [req.user.sub]);
...
router.get("/plans/public/:id", async (req, res) => {
    try {
        const { rows } = await pool_1.pool.query(`SELECT rp.id, rp.description, rp.amount, rp.interval, rp.day_of_week, rp.day_of_month, rp.status,
        ...
        WHERE rp.id = $1 AND rp.status = 'active'`, [req.params["id"]]);
        if (!rows[0])
...
router.get("/subscriptions", auth_1.authenticate, async (req, res) => {
    try {
        const { rows } = await pool_1.pool.query(`SELECT s.*, rp.description AS plan_description, rp.amount AS plan_amount,
        ...
        ORDER BY s.created_at DESC`, [req.user.sub]);
...
router.post("/plans/:id/subscribe", auth_1.authenticate, async (req, res) => {
    ...
    try {
        const { rows: planRows } = await pool_1.pool.query("SELECT * FROM recurring_plans WHERE id = $1 AND status = 'active'", [req.params["id"]]);
        if (!planRows[0])
            return res.status(404).json({ error: "Plan not found or not active" });
        ...
        const { rows } = await pool_1.pool.query(`INSERT INTO subscriptions (plan_id, subscriber_user_id, next_run_at)
       VALUES ($1,$2,$3)
       ON CONFLICT (plan_id, subscriber_user_id) DO UPDATE
         SET status = 'active', next_run_at = EXCLUDED.next_run_at
       RETURNING *`, [plan.id, req.user.sub, nextRunAt.toISOString()]);
...
router.delete("/subscriptions/:id", auth_1.authenticate, async (req, res) => {
    try {
        const { rowCount } = await pool_1.pool.query("UPDATE subscriptions SET status = 'canceled' WHERE id = $1 AND subscriber_user_id = $2", [req.params["id"], req.user.sub]);
        if (!rowCount)
...
router.patch("/plans/:id/pause", auth_1.authenticate, async (req, res) => {
    try {
        const { rowCount } = await pool_1.pool.query("UPDATE recurring_plans SET status = 'paused' WHERE id = $1 AND creator_user_id = $2", [req.params["id"], req.user.sub]);
        if (!rowCount)
