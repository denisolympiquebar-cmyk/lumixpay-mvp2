...
router.get("/me/accounts", auth_1.authenticate, async (req, res) => {
    try {
        const accounts = await LedgerService_1.ledgerService.getAccountsWithBalances(req.user.sub);
...
router.get("/me/accounts/:accountId/history", auth_1.authenticate, async (req, res) => {
    const { accountId } = req.params;
    const limit = Math.min(parseInt(String(req.query["limit"] ?? "50"), 10), 200);
    const offset = parseInt(String(req.query["offset"] ?? "0"), 10);
    try {
        const rawEntries = await LedgerService_1.ledgerService.getLedgerHistory(accountId, limit, offset);
        ...
        if (transferEntries.length > 0) {
            ...
            const counterpartyAccountIds = [
                ...new Set(transferEntries.map((e) => e.credit_account_id === accountId ? e.debit_account_id : e.credit_account_id)),
            ];
            const { rows } = await pool_1.pool.query(`SELECT a.id AS account_id, u.id AS user_id, u.username
           FROM accounts a
           JOIN users u ON u.id = a.user_id
          WHERE a.id = ANY($1::uuid[])`, [counterpartyAccountIds]);
        ...
        const entries = rawEntries.map((e) => {
            if (e.entry_type !== "transfer")
                return e;
            const counterpartyAccountId = e.credit_account_id === accountId ? e.debit_account_id : e.credit_account_id;
