...
router.get("/reconciliation", auth_1.authenticate, (0, auth_1.requireRole)("admin"), async (_req, res) => {
    try {
        const totalQ = await pool_1.pool.query(`SELECT
         COALESCE(SUM(amount), 0)::text AS debits,
         COALESCE(SUM(amount), 0)::text AS credits
       FROM ledger_entries`);
        const t = totalQ.rows[0] ?? { debits: "0", credits: "0" };
        const byAssetQ = await pool_1.pool.query(`SELECT
         le.asset_id,
         a.currency_code,
...
router.get("/", auth_1.authenticate, (0, auth_1.requireRole)("admin"), async (req, res) => {
    const { user_id, email, username, from, to, entry_type } = req.query;
    ...
    try {
        ...
        if (!resolvedUserId && (email || username)) {
            const { rows } = await pool_1.pool.query("SELECT id FROM users WHERE email = $1 OR username = $2", [email ?? null, username ?? null]);
            if (rows[0])
        ...
        const { rows } = await pool_1.pool.query(`SELECT le.*, ast.currency_code, ast.display_symbol
         FROM ledger_entries le
         JOIN assets ast ON ast.id = le.asset_id
        ${where}
        ORDER BY le.created_at DESC
        LIMIT $${p} OFFSET $${p + 1}`, params);
...
//# sourceMappingURL=admin-ledger.js.map