...
router.get("/", auth_1.authenticate, (0, auth_1.requireRole)("admin"), async (req, res) => {
    ...
    try {
        const { rows } = await pool_1.pool.query(`SELECT u.id, u.email, u.full_name, u.username, u.role, u.created_at,
        ...
        LIMIT $2 OFFSET $3`, [search ? `%${search}%` : "", limit, offset]);
...
router.get("/:id", auth_1.authenticate, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
        const { rows: userRows } = await pool_1.pool.query("SELECT id, email, full_name, username, role, created_at FROM users WHERE id = $1 AND role != 'system'", [req.params["id"]]);
        if (!userRows[0])
            return res.status(404).json({ error: "User not found" });
        const { rows: accountRows } = await pool_1.pool.query(`SELECT a.id, a.label, a.asset_id, a.created_at,
        ...
        WHERE a.user_id = $1`, [req.params["id"]]);
