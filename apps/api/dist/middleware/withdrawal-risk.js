...
exports.withdrawalRiskGuard = withdrawalRiskGuard;
...
for (const pair of rawOverrides.split(",").map((v) => v.trim()).filter(Boolean)) {
    ...
    if (!userId || !tier)
        continue;
    if (tier === "retail" || tier === "business" || tier === "institutional")
        overrideMap[userId] = tier;
}
const softDailyLimitByTier = {
...
const highValueHits = new Map();
function resolveTier(req) {
    const userId = req.user?.sub ?? "";
    if (userId && overrideMap[userId])
        return overrideMap[userId];
 ...
 */
function withdrawalRiskGuard(req, res, next) {
...
//# sourceMappingURL=withdrawal-risk.js.map